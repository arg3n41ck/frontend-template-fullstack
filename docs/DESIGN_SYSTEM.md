...
- `apps/web/src/shared/config/styles/palette.css` is the single frontend color source of truth: primitive brand/neutral/status values map to light/dark semantic tokens. `global.css` only exposes those semantics to Tailwind.
...
- `apps/web/src/shared/ui/shadcn` contains registry primitives only. Reusable product compositions belong outside that folder; feature-specific UI stays with the feature.
